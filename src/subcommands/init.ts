...
export async function initHandler(): Promise<void> {
  ...
  if (!fs.existsSync(annotatedResolved)) {
    ...
    console.log(`${CLIColor.FgGreen4}✔${CLIColor.Reset} Created empty annotated mods file at: ${CLIColor.FgCyan1}${annotatedResolved}${CLIColor.Reset}`);
  } else {
    console.log(`${CLIColor.FgYellow8}⚠${CLIColor.Reset} Annotated mods file already exists at: ${CLIColor.FgCyan1}${annotatedResolved}${CLIColor.Reset}, skipping.`);
  }

  console.log(`\n${CLIColor.FgGreen3}Initialization complete!${CLIColor.Reset}`);
  console.log(`${CLIColor.FgGray19}Run ${CLIColor.BgBlack1}${CLIColor.FgGreen1} packscripts refresh ${CLIColor.Reset}${CLIColor.FgGray19} to build your annotated mods list.${CLIColor.Reset}`);
}
